<template>
    <form-layout @submit="onSubmit">
        ...
        <nav-link
            :route="{ name: ERouteName.DASHBOARD_SIGNUP }"
            fluid>
            <app-button
                :type="EButtonType.BUTTON"
                outlined
                fluid>Зарегистрироваться</app-button>
        </nav-link>
    </form-layout>
</template>
...
const onSubmit = handleSubmit(async values => {
    const response = await userApi.login(values[EFormFieldId.EMAIL], values[EFormFieldId.PASSWORD]);
